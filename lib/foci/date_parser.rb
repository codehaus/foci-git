class Foci::DateParser
  ...
  def self.parse(datetime)
    ...
    timezone_offset = timezone_dir * (timezone_hour * 3600 + timezone_minute * 60)
    
    #puts datetime
    #puts "#{day} #{month_name}(#{month}) #{year} #{hour} #{minute} #{second} #{timezone_text} (#{timezone_offset})"
    #DateTime.strptime(datetime, "%d/%b/%Y:%H:%M:%S %z")
    
    #time = Time.utc(second, minute, hour, day, month, year, 0, 0, false, timezone)
    #puts time.zone
